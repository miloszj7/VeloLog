...
- `gpx/` — the third: upload, parse, store and download a trip's GPX file, derive and store its statistics (distance, recorded time, elevation gain/loss), build the map config and stats blob its detail page renders, and **own the stored file's lifecycle**. A `post_delete` receiver on `GpxTrack` (`gpx/signals.py`, registered in `gpx/apps.py`) is the single place a `.gpx` file is removed from storage, so every deletion path is covered: trip cascade, admin `delete_selected`, replacement upload, and any bare `QuerySet.delete()`. Do not reintroduce inline storage deletes — registering that receiver is also what stops the collector fast-deleting `GpxTrack` rows, and without materialized rows a trip cascade has no instance to read a storage key from. The same listener check also disables the collector's field-deferral optimization, so a cascade now selects whole `GpxTrack` rows including the `points` blob — which is why the scheduled callback closes over the pk and storage key rather than the instance; an instance closure would hold every one of those rows resident past commit. The receiver schedules its work with `transaction.on_commit`, so a test asserting a file was removed must wrap the request in `django_capture_on_commit_callbacks(execute=True)` or it passes while proving nothing. The receiver covers every path that *deletes* a row; an upload whose transaction rolls back after the file is written leaves an orphan no delete ever fires for, which is the one hole in the end-to-end claim above (roadmap **E-11**).
