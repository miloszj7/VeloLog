...
**Contract**: Register `Trip` with `@admin.register(Trip)` and a `TripAdmin(admin.ModelAdmin[Trip])` — `ModelAdmin` is not subscriptable at runtime under django-stubs (only `UserCreationForm` carries `__class_getitem__`; `ModelForm` isn't subscriptable either, see item 1 below), so this needs the same `TYPE_CHECKING` shim as the generic CBVs — carrying `list_display = ("name", "date", "owner")`. No other customization.
...
**Contract**: `TripForm(forms.ModelForm[Trip])` — `ModelForm` is not subscriptable at runtime under django-stubs, same as the generic views; only `UserCreationForm` subscripts directly because it alone carries `__class_getitem__` (`accounts/forms.py:7` shows that exception, not the general case). Use the same `TYPE_CHECKING` shim as the generic CBVs. `Meta.model = Trip`, `Meta.fields = ("name", "date", "description")` as a tuple. A `widgets` entry giving `date` a `forms.DateInput(attrs={"type": "date"})` so browsers show a native date picker — this is the repo's first widget override and is justified because a bare text input for a date is a usability failure, not a styling preference.
