...
| `uv run python manage.py backfill_gpx_stats` | Refill `GpxTrack` statistics **and stage instants** (`started_at`/`ended_at`) from the stored files, for rows where the statistics are null (`--all` reprocesses every row). The default filter is `distance_meters__isnull=True` and stays that way deliberately: an untimed file's instants are legitimately null forever, so a filter on them would re-select the same rows on every run and the pending tally would never converge. **`--all` is therefore the invocation that refills instants on a row whose statistics are already present.** The documented recovery path when migration `0003` ran against a misconfigured `MEDIA_ROOT` and filled nothing — a migration cannot be re-applied once recorded. Per-row failures are a tally, not a crash |
